...
try:
    from src.ops.fftconv import fftconv_func  # fftconv_ref
except ImportError:
    fftconv_func = None
...
class ExponentialModulation(OptimModule):
    def __init__(self, d_model, hidden_ssm_dim=64, fast_decay_pct=0.3, slow_decay_pct=1.5, target=1e-2, modulation_lr=0.0,
                 modulate: bool = True, shift: float = 0.0, **kwargs):
        super().__init__()
        self.modulate = modulate
        self.shift = shift
        max_decay = math.log(target) / fast_decay_pct
        min_decay = math.log(target) / slow_decay_pct
        # deltas = torch.linspace(min_decay, max_decay, d_model)[None, None] # todo
        deltas = torch.linspace(min_decay, max_decay, d_model)[None, None, :, None]
        deltas = deltas.expand(deltas.shape[:3] + (hidden_ssm_dim, ))
...
class HyenaFilter(OptimModule):
    def __init__(
        ...
        self.hidden_ssm_dim = hidden_ssm_dim
        self.use_bias = bias
        ...
        # final linear layer
        # self.implicit_filter.append(nn.Linear(order, d_model, bias=False)) # todo
        self.implicit_filter.append(nn.Linear(order, d_model * hidden_ssm_dim, bias=False))

        # self.modulation = ExponentialModulation(d_model, **kwargs) # todo
        self.modulation = ExponentialModulation(d_model, hidden_ssm_dim, **kwargs)
...
class HyenaOperator(nn.Module):
    ...
    def setup_filters(self, filter_cls, filter_args):
        ...
        self.short_filter = nn.Conv1d(in_channels=total_width, out_channels=total_width, kernel_size=self.short_filter_order,
                                      groups=total_width, padding=self.short_filter_order - 1)

        filter_cls = instantiate(registry.layer, filter_cls, partial=True)

        self.filter_fn = filter_cls(self.head_dim * self.inner_factor * (self.order - 1), order=self.filter_order,
                                    hidden_ssm_dim=self.hidden_ssm_dim, seq_len=self.l_max, channels=1,
                                    dropout=self.filter_dropout, **filter_args)
        if self.jit_filter:
    ...
    def forward(self, u, *args, **kwargs):
        l_len = u.size(-2)
        l_filter = min(l_len, self.l_max)
        u = self.in_proj(u)
        u = rearrange(u, 'b l d -> b d l')

        uc = self.short_filter(u)[..., :l_filter]

        uc = rearrange(uc, 'b (ho v) (z l) -> b ho v z l', z=self.num_blocks, ho=self.num_heads,
        ...
        *x, v = uc.split(self.d_model, dim=2)
        ...
        for o, x_i in enumerate(reversed(x[1:])):
            ...
            if self.post_order_ffn:
                w = self.ord_proj_w[o]
                v = mul_sum(rearrange(w, 'h1 h2 -> 1 h1 h2 1 1 1'), rearrange(v, 'b h v z l -> b h 1 v z l'))
